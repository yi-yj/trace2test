...
本文件是本仓库所有开发工作的总指导。后续参与者应先阅读本节，再按下方完整构建说明实施。

## 执行约束

- 以 MVP 的“轨迹采集—诊断—聚类—回归”闭环为最高优先级，严格遵循第 30 节的实施顺序。
- 当前阶段只实现当前里程碑的验收条件；未满足阶段门禁时，不提前开发 WebArena、UI-TARS 或 OSWorld。
- 主要代码在 WSL2/Linux 环境运行；Python 使用 3.11，依赖优先使用 `uv` 管理，前端使用 Node.js 20 LTS 与 pnpm。
- 外部框架必须通过 adapter 隔离，并固定 commit/tag；统一轨迹协议必须保持独立版本。
- 每项功能同时实现与风险相称的测试。至少覆盖 schema 单元测试、adapter/runner 集成测试和关键失败类型的 golden fixtures。
- 判断任务成功时优先使用确定性 verifier；LLM Judge 只能作为有证据约束的辅助诊断，不得当作 ground truth。
- 不提交密钥、私有思维链、未脱敏轨迹或大体积运行产物。凭证只放在被 Git 忽略的 `.env` 中，并维护 `.env.example`。
- 修改架构、轨迹 schema、依赖版本或 release gate 时，同步更新相关文档和可复现清单。
- 每次提交前运行受影响范围的格式检查、静态检查和测试，并在交付说明中报告未验证项。
- 如本文件内部内容存在冲突，依次以“明确不做”边界、MVP 范围、当前阶段验收条件、最终验收指标为准。

## 原始构建说明

以下内容由 `项目3_Trace2Test_GUI_Agent持续评测平台_构建说明.md` 完整迁入，作为架构、范围、数据模型、阶段计划与验收标准的依据。
