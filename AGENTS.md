...
9. 完成明确范围的开发任务并通过相关测试后，可主动创建本地 Git commit；默认不得执行 `git push`，只有用户明确要求当次推送时才可推送远端。
10. 用户明确要求推送时，必须先获取远端最新状态，解决所有合并或变基冲突，再重新运行相关测试；禁止使用强制推送覆盖他人历史，除非用户明确授权。
11. 合并或发布前必须通过适用的 CI 与 release gate；P0/P1 历史故障复发、新致命故障簇或安全违规默认阻止发布。
